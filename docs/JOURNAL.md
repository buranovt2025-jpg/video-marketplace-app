...
### Автоматизация (важно)
- В ветку также попадали файлы `.cursorrules` и `automation/*` (auto-commit/watch скрипты) из коммита `5ad0488`.
- Они содержали **рискованные/неверные инструкции** (автопилот коммитов/пушей, “main деплоится”, root-пользователь, long-running watcher).
- Эти файлы **удалены из ветки**, чтобы не путать процесс и не создавать рисков.
- Поддерживаемая автоматизация деплоя для проекта: **GitHub Actions workflow** `.github/workflows/deploy_web.yml` (SSH деплой на сервер).


---

## 2025-12-31 - Тестирование после деплоя d757618

### Проведено полное тестирование по ролям

**Деплой:** d75761862a10d498ded51c0cd4eccd6416eaeaac  
...
#### 0) Базовая проверка
- ✅ Build ID подтвержден на сервере
- ✅ Console: нет критических ошибок
- ✅ Network: основные ресурсы загружаются (200 OK)

#### 1) Гость (без логина)
- ✅ Лента грузится и скроллится
- ✅ Кнопка "Вход" открывает экран логина
- ✅ Вкладка "Заказы" показывает сообщение "Войдите" без крашей

#### 2) Buyer (buyer@demo.com)
- ✅ Логин успешен (POST /api/auth/login → 200 OK)
- ✅ access_token присутствует в ответе
- ✅ Нет 401 ошибок после логина
- ✅ Заказы загружаются (GET /api/orders → 200)
- ✅ Checkout работает, заказ успешно создан (POST /api/orders → 200)
- ✅ Корзина открывается, оформление не зависает

#### 3) Seller (seller@demo.com)
- ✅ Кабинет продавца открывается
- ✅ "Мои товары" загружаются (3 товара)
- ✅ Создание товара работает
- ✅ Битый URL изображения не приводит к крашу
- ✅ Заказы продавца загружаются
- ✅ Детали заказа отображаются корректно

#### 4) Courier (courier@demo.com)
- ✅ Логин успешен
- ✅ Вкладки "Доступные", "Активные", "История" загружаются без ошибок
- ⚠️ Детали заказа не протестированы (нет доступных заказов)

#### 5) Admin (admin@demo.com)
- ✅ Логин успешен
- ✅ Admin Dashboard открывается, статистика отображается
- ✅ Разделы "Пользователи", "Заказы", "Контент" загружаются корректно

### ⚠️ Некритичные проблемы:

1. **Внешние изображения не загружаются:**
   - unsplash.com, gravatar.com, pravatar.cc → CORS/404 ошибки
   - Не влияет на работу приложения

2. **Ошибки Firebase:**
   - Предупреждения инициализации Firebase
   - Не влияют на функциональность

3. **Отсутствие WebGL:**
   - Приложение переключается на CPU-рендеринг
   - Работает нормально
...
- Авторизация всех ролей работает
- Заказы создаются и отображаются
- Checkout не зависает
- Нет серых экранов
- Нет 401/500 ошибок после логина
- Console без критических ошибок

**Предыдущие критические проблемы ИСПРАВЛЕНЫ:**
- ✅ Белый экран → исправлен
- ✅ Вкладка "Доступные" у курьера → работает
- ✅ Страница "Заказы" у покупателя → работает
- ✅ Зависание при checkout → исправлено

**Отчет:** Детальный отчет сохранен в `gogomarket_test_d757618.md`
...
- Добавлены подсказки **продавцов по имени** прямо в строке поиска (подгружается `/api/sellers`; клик по продавцу ставит фильтр `seller_id`).
- В поиске добавлен индикатор активных фильтров + быстрый “Сбросить фильтры”.
- Рилсы/истории: исправляю, чтобы визуально были ближе к TikTok/Instagram:
  - `marketplace_home_screen.dart`: превью рилса теперь показывает `thumbnail_url` + кнопка “Смотреть” открывает полноэкранный вертикальный просмотр.
  - `reels_viewer_screen.dart`: новый экран полноэкранного просмотра рилсов (PageView вертикально + overlay).
  - `story_viewer_screen.dart`: видео-истории теперь реально проигрываются (не просто иконка), длительность прогресса для видео увеличена, и включён свайп между сторис.
  - `video_player_iten.dart`: видео теперь `BoxFit.cover`, loop, и можно ставить паузу/продолжать по тапу (как в TikTok).
...
## Сессия 31 декабря 2025 — 1 января 2026 (Cursor) — Локализация “до конца” + курьерские экраны

### Что сделано
- Долокализованы экраны **Auth + Profile** (убраны хардкод‑строки, добавлены ключи в `lib/l10n/app_translations.dart`):
  - `marketplace_login_screen.dart`, `marketplace_register_screen.dart`, `forgot_password_screen.dart`
  - `edit_profile_screen.dart`
- Долокализованы экраны **Seller**:
  - `create_product_screen.dart` (категории через ключи, ошибки/подсказки/лейблы)
  - `my_products_screen.dart` (пустые состояния, кнопки, диалоги удаления)
  - `seller_analytics_screen.dart` (периоды, метрики, пустые состояния)
  - `create_reel_screen.dart`, `create_story_screen.dart` (остаточные строки)
- Долокализованы экраны **Courier**:
  - `courier_home_screen.dart` (табы, пустые состояния, подтверждение “взять заказ”, форматирование)
  - `courier_order_detail_screen.dart` (статусы, навигация/чат, товары/оплата, сообщения)
